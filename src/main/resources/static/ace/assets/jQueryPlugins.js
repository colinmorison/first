 ...
 * refType种类：notNull、notNull(id1,id2..)、phone、equalWith(id)、int+、int-、int、maxLength(位数)、length(位数)、length[int/string](位数)、double+(整数位,小数位)、double-(整数位,小数位)、double(整数位,小数位)、自定义类型
...
;(function($){
	...
	var allTypeDefault = 'notNull、notNull(id1,id2..)、int+、int-、int、maxLength(位数)、length(位数)、length[int/string](位数)、double+(整数位,小数位)、double-(整数位,小数位)、double(整数位,小数位)、自定义类型';
	
	//[样例]扩增校验函数【equalWith】:校验是否相等
	configdefault.addCheck["equalWith(id)"] = {
		"rex" : /^equalWith\(([0-9a-zA-Z]+)\)$/,
		"checkFunction" : function(jTarget,value, rexGroups){
	        var array = new Array('y','输入不一致'),
	        	id = rexGroups[1];
	        if(value==$("#"+id).val()){
	            return array[0];
	        }else {
	...
	//[样例]扩增校验函数【phone】
	configdefault.addCheck["phone"] = {
		"rex": /^phone$/,
		"checkFunction" : function(jTarget,value, rexGroups){
	        var array = new Array('y','请输入正确的手机号');
	...
	var getAllType = function(thisdefault){
		var allType = [allTypeDefault];
		for (var key in thisdefault.addCheck){
			allType.push(key);
		}
		return allType.join("、");
	};
	...
	var isRegexCheck = function(type){
		try{
			var custCheckObj = $.parseJSON(type);
			if(typeof custCheckObj["warnmsg"] != "string"){
				return false;
			}
			//回调函数或者正则表达式
			var regex = custCheckObj["regex"];
	...
	var regexCheck = function(type,jTarget){
		var custCheckObj = $.parseJSON(type);
		var checkResult = true;
		//回调函数或者正则表达式
		checkResult = (new RegExp(custCheckObj["regex"],"g")).test(jTarget.val());
		if(false == checkResult){
			return custCheckObj["warnmsg"];
		}
		return 'y';
	};
	var isCustCheck = function(type){
		try{
			...
			if($.isFunction(eval(custCheckObj["checkfun"]))){
				return true;
			}
			return false;
		}catch(e){
	...
	var custCheck = function(type,jTarget){
		var custCheckObj = $.parseJSON(type);
		var checkResult = true;
		var checkfun = eval(custCheckObj["checkfun"]);
	...
	var checkType = function(name,jTarget,thisdefault){
		...
		for(i = 0; i < types.length; i++){
			...
			if((tmp = notNullRex.exec(type)) != null){//非空【notNull】
				...
				}else if(isRegexCheck(type) == true){//自定义类型{"regex":"^[0-9]+$","warnmsg":"非数字"}
					msg = regexCheck(type,jTarget);
				}else if(isCustCheck(type) == true){//自定义类型{"checkfun":"check1","warnmsg":"长度小于3"}
					msg = custCheck(type,jTarget);
				}else{
					typeError = true;
					addChecks = thisdefault.addCheck;
					for(objKey in addChecks){
						addCheck = addChecks[objKey];
						if((tmp = addCheck["rex"].exec(type)) == null){
							continue;
						}
						msg = addCheck["checkFunction"].apply(jTarget[0], [jTarget, value, tmp]);
						typeError = false;
						break;
					}
					if(typeError){
						throw new Error("dom对象name["+jTarget.attr("name")+"]的"+thisdefault.refTypePro+"属性["+type+"]配置错误，请从以下样式选择"+getAllType(thisdefault));
					}
				}
			}
	...
	var init = function(thisdefault){
		var debugMsg, inputs = thisdefault.thiz.find("input[type='password'],input[type='text'],input[type='radio'],input[type='checkbox'],input[type='file'],select,textarea");
